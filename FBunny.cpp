...
std::vector<FString> NameArray{ "Rudi", "Adi", "Rex", "Morty", "Rick", "Alex", "Echo", "Niko", "Elijah", "Niklaus", "Biggie" };
std::vector<FString> ColorArray{ "Smoke White", "Gray", "Spotted", "Black", "White"};


	FString FBunny::Random_Name() {
		int32 ret = rand() % NameArray.size();
		return NameArray[ret];
	}
	



	FString FBunny::Random_Color() {
		int32 ret = rand() % ColorArray.size();
		return ColorArray[ret];
	}

	FString FBunny::Random_Sex()
	{
		int32 random = rand() % 100;
		if (random < 50) { return "Male"; }
		else { return "Female"; };
	}
